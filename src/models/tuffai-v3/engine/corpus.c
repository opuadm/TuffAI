...
static void gen_echo_keyword(const char *input, char *out, int out_size) {
    ...
    v3_extract_keyword_ext(input, keyword, sizeof(keyword));
...
static void gen_confusion(const char *input, char *out, int out_size) {
    ...
    case 2:
        ...
        break;
    case 3:
        v3_extract_keyword_ext(input, confused_term, sizeof(confused_term));
        snprintf(out, out_size, "%s?", confused_term);
        break;
    case 4:
...
static void gen_code(const char *input, char *out, int out_size) {
    ...
    switch (code_lang) {
    case CODE_LANG_C:   snippet = v3_knowledge_random(&v3_know_code_c); break;
    case CODE_LANG_PY:  snippet = v3_knowledge_random(&v3_know_code_py); break;
    case CODE_LANG_JS:  snippet = v3_knowledge_random(&v3_know_code_js); break;
    case CODE_LANG_MISC:snippet = v3_knowledge_random(&v3_know_code_misc); break;
    default:            snippet = v3_knowledge_random_code(); break;
    }
    ...
    if (style < 4) {
        ...
        switch (code_lang) {
        case CODE_LANG_C:   snippet2 = v3_knowledge_random(&v3_know_code_c); break;
        case CODE_LANG_PY:  snippet2 = v3_knowledge_random(&v3_know_code_py); break;
        case CODE_LANG_JS:  snippet2 = v3_knowledge_random(&v3_know_code_js); break;
        case CODE_LANG_MISC:snippet2 = v3_knowledge_random(&v3_know_code_misc); break;
        default:            snippet2 = v3_knowledge_random_code(); break;
        }
        written = snprintf(out, out_size, "%s\n\n%s", snippet, snippet2);
        (void)written;
    }
...
static void gen_multilang(const char *input, char *out, int out_size) {
    ...
    if (style < 2) {
        phrase = v3_knowledge_random_phrase();
        snprintf(out, out_size, "%s", phrase);
    } else if (style == 2 && lang_hint == 1) {
        phrase = v3_knowledge_random(&v3_know_phrases_pl);
        ...
        phrase = v3_knowledge_random(&v3_know_phrases_ru);
        ...
        phrase = v3_knowledge_random(&v3_know_phrases_zh);
        ...
        if (got) {
            ...
            snprintf(out, out_size, "%s", v3_knowledge_random_phrase());
        }
    } else {
        got = web_fetch_random_any_lang(search_text, sizeof(search_text));
        if (got) {
...
static void gen_knowledge(const char *input, char *out, int out_size) {
    ...
    if (style == 0) {
        entry = v3_knowledge_search(&v3_know_tech, keyword);
        snprintf(out, out_size, "%s", entry);
    } else if (style == 1) {
        entry = v3_knowledge_random(&v3_know_general);
        snprintf(out, out_size, "%s", entry);
    } else if (style == 2) {
        entry = v3_knowledge_random_any();
        written = snprintf(out, out_size, "%s?\n\n%s", keyword, entry);
        (void)written;
    } else if (style == 3) {
        entry = v3_knowledge_random_any();
...
int v3_pick_response_mode(int pattern, const Features *feat, int turn_count) {
    ...
    if (rand() % 100 < 3) return RESP_KNOWLEDGE;
    if (rand() % 100 < 2) return RESP_MULTILANG;
    if (rand() % 100 < 2) return RESP_KNOWLEDGE;
    ...
    if (pattern == PAT_MATH) {
        ...
        if (r < 50) return RESP_KNOWLEDGE;
        if (r < 58) return RESP_KNOWLEDGE;
        if (r < 68) return RESP_SEARCH_CONFUSED;
        if (r < 78) return RESP_KNOWLEDGE;
        if (r < 85) return RESP_KNOWLEDGE;
        if (r < 92) return RESP_KNOWLEDGE;
        return RESP_CONFUSION;
    }

    if (pattern == PAT_TECH) {
        if (r < 15) return RESP_KNOWLEDGE;
        if (r < 25) return RESP_KNOWLEDGE;
        if (r < 35) return RESP_KNOWLEDGE;
        if (r < 48) return RESP_SEARCH_MANGLE;
        if (r < 58) return RESP_CODE_GEN;
        if (r < 68) return RESP_KNOWLEDGE;
        if (r < 78) return RESP_SEARCH_DRIFT;
        if (r < 85) return RESP_KNOWLEDGE;
        if (r < 92) return RESP_KNOWLEDGE;
        return RESP_MARKOV;
    }
    ...
    if (pattern == PAT_QUESTION || pattern == PAT_TIME) {
        if (r < 12) return RESP_KNOWLEDGE;
        if (r < 20) return RESP_KNOWLEDGE;
        if (r < 30) return RESP_SEARCH_CONFUSED;
        if (r < 40) return RESP_KNOWLEDGE;
        if (r < 48) return RESP_KNOWLEDGE;
        if (r < 56) return RESP_ECHO_KEYWORD;
        if (r < 64) return RESP_SEARCH_DRIFT;
        if (r < 72) return RESP_KNOWLEDGE;
        if (r < 78) return RESP_KNOWLEDGE;
        if (r < 84) return RESP_MULTILANG;
        if (r < 90) return RESP_KNOWLEDGE;
    ...
    if (pattern == PAT_EMOTIONAL) {
        ...
        if (r < 42) return RESP_KNOWLEDGE;
        if (r < 52) return RESP_CONFUSION;
        if (r < 62) return RESP_MULTILANG;
        if (r < 72) return RESP_SEARCH_CONFUSED;
        if (r < 82) return RESP_KNOWLEDGE;
    ...
    if (r < 8) return RESP_KNOWLEDGE;
    if (r < 14) return RESP_SEARCH_CONFUSED;
    if (r < 22) return RESP_KNOWLEDGE;
    if (r < 28) return RESP_KNOWLEDGE;
    if (r < 36) return RESP_ECHO_KEYWORD;
    if (r < 42) return RESP_KNOWLEDGE;
    if (r < 50) return RESP_SEARCH_DRIFT;
    if (r < 56) return RESP_KNOWLEDGE;
    if (r < 62) return RESP_SEARCH_MANGLE;
    if (r < 67) return RESP_MULTILANG;
    if (r < 72) return RESP_MULTILANG;
    if (r < 78) return RESP_CONFUSION;
    if (r < 83) return RESP_KNOWLEDGE;
    if (r < 88) return RESP_MARKOV;
    if (r < 92) return RESP_SEARCH_FOREIGN;
    if (r < 96) return RESP_KNOWLEDGE;
    return RESP_ECHO_MANGLE;
}
...
int v3_generate_corpus_response(int mode, const char *input, int pattern, const Features *feat, char *out, int out_size) {
    ...
    case RESP_ECHO_MANGLE:
        v3_scramble_words(input, out, out_size);
        return 1;
